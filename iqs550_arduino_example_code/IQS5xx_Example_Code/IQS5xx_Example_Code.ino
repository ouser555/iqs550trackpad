...
void setup()
{
  ...
  //while(1){Serial.println("first command");};
}
...
void loop()
{
	...
	I2C_Read(GestureEvents0_adr, &Data_Buff[0], 44); //從0x000D往下讀，共44byte
	...
	if((Data_Buff[3] & SNAP_TOGGLE) != 0)
	{
		...
		} //8bit轉16bit，30byte=> (15 x 16bit)

	}
	//
	// Terminate the communication session, so that the IQS5xx can continue 
	// with sensing and processingc:\Users\shiva\Downloads\Compressed\IQS5xx Bootloader Example v1.1\IQS5xx_Example_Code\HEX_Array.h.bak1
	//
	Close_Comms();
	//
	// Process received data 
	//
	Process_XY(); //秀出資料主要就是這個
}
